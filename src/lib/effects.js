export const FINLAND_BACKGROUNDS = [
  ...
  "https://commons.wikimedia.org/wiki/Special:FilePath/Ikaalinen_-_lake_and_forest.jpg?width=900",
];
...
export function installGlobalHaptics() {
  ...
  const onPointerDown = (event) => {
    ...
    window.setTimeout(() => document.documentElement.style.setProperty("--reactive-press", "0"), 140);
  };
...
export function installReactiveUI() {
  ...
  function update() {
    ...
    const momentum = Math.max(0, Math.min(1, delta / 90));
    root.style.setProperty("--scroll-momentum", momentum.toFixed(3));
    root.style.setProperty("--reactive-glow", (0.12 + momentum * 0.36).toFixed(3));
    root.style.setProperty("--parallax-y", `${Math.max(-18, Math.min(18, y * 0.025))}px`);
    root.dataset.scrollDirection = rawDelta >= 0 ? "down" : "up";

    const now = Date.now();
    if (delta > 42 && now - lastHaptic > 190) {
      haptic("scroll");
      lastHaptic = now;
    }
