...
const VERSION = "NEW POST SAFE 2026-04-28";
...
async function rewardPostSafely(userId) {
  if (!userId) return;
  try {
...
export default function NewPostPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      await rewardPostSafely(user.id);
      setContent("");
      setSuccessMessage("Perustelu julkaistu.");
      navigate("/feed");
    } catch (err) {
      await logClientError(supabase, {
        source: "new_post",
        message: err?.message || "Postauksen lähetys epäonnistui.",
        details: { content: String(content || "").slice(0, 250) },
        user_id: user?.id,
      });
      ...
      <header className="mx-auto max-w-md">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h1 className="mt-2 text-4xl font-black tracking-tight">Uusi perustelu</h1>
          </div>
      ...
      <main className="mx-auto mt-6 max-w-md">
        <form onSubmit={handleSubmit} className="rounded-[34px] border border-white/10 bg-white/10 p-5 shadow-2xl backdrop-blur-xl">
          ...
          <textarea
            ...
            className="mt-4 min-h-[260px] w-full resize-none rounded-[28px] border border-white/10 bg-black/25 p-5 text-lg font-bold leading-relaxed text-white outline-none placeholder:text-white/25 focus:border-cyan-300/50"
            maxLength={1000}
          />
          ...
          >
            {posting ? "Julkaistaan..." : validation.ok ? "Julkaise perustelu" : "Kirjoita perustelu"}
          </button>
        </form>
      </main>
    </div>
