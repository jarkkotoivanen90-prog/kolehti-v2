...
function Styles() {
  return (
    <style>{`
      ...
      @keyframes cardGlow{0%,100%{box-shadow:0 0 22px rgba(34,211,238,.12),0 0 44px rgba(255,255,255,.03)}50%{box-shadow:0 0 34px rgba(34,211,238,.24),0 0 70px rgba(250,204,21,.10)}}
      ...
      @keyframes shine{0%{transform:translateX(-150%) skewX(-18deg);opacity:0}26%{opacity:.55}58%{transform:translateX(150%) skewX(-18deg);opacity:0}100%{transform:translateX(150%) skewX(-18deg);opacity:0}}
      .nav-alive{animation:navAlive 2.5s ease-in-out infinite}.plus-pulse{animation:plusPulse 2.25s ease-in-out infinite}.card-glow{animation:cardGlow 3s ease-in-out infinite}.xp-pop{animation:xpPop 1.8s ease both}.shine{position:relative;overflow:hidden}.shine:after{content:"";position:absolute;inset:-40px;background:linear-gradient(90deg,transparent,rgba(255,255,255,.18),transparent);animation:shine 5.2s ease-in-out infinite;pointer-events:none}.safe-text{min-width:0;overflow:hidden;text-overflow:ellipsis}.snap-card{scroll-snap-align:start;scroll-snap-stop:always}
    `}</style>
  );
}
      ...
      <div className="relative rounded-[34px] border border-cyan-200/20 bg-[#061126]/92 px-4 pb-4 pt-3 shadow-2xl shadow-cyan-500/15 backdrop-blur-2xl">
        ...
        <div className="grid grid-cols-5 items-end text-center text-[11px] font-black">
          {items.map((item) => {
            ...
            if (item.fab) {
              return (
                <Tap onPulse={onPulse} key={item.to} to={item.to} className="-mt-12 flex flex-col items-center">
            ...
            return (
              <Tap onPulse={onPulse} key={item.to} to={item.to} className={`relative flex flex-col items-center gap-1.5 rounded-3xl px-1 py-2 active:scale-95 ${active ? "text-cyan-100" : "text-white/52"}`}>
...
function FeedTopCard({ watchPot, activePlayers, postCount, totalWatches, topPost }) {
  return (
    <div className="shine card-glow w-full rounded-[36px] border border-cyan-200/20 bg-white/[0.075] p-5 shadow-2xl backdrop-blur-2xl">
      <p className="safe-text text-xs font-black uppercase tracking-[0.24em] text-cyan-100/80">🇫🇮 Clean feed · insanity lite</p>
      ...
      <p className="mt-3 text-sm font-bold leading-snug text-white/70">Postaus, ääni ja potti ovat pääroolissa. Efektit tukevat käyttöä eivätkä sotke lukemista.</p>

      <div className="mt-5 grid grid-cols-3 gap-2 text-center text-xs font-black">
        <div className="rounded-2xl bg-black/30 p-3"><div className="safe-text text-white/45">Pelaajat</div><div className="mt-1 text-xl">{activePlayers}</div></div>
        <div className="rounded-2xl bg-black/30 p-3"><div className="safe-text text-white/45">Postit</div><div className="mt-1 text-xl">{postCount}</div></div>
        <div className="rounded-2xl bg-black/30 p-3"><div className="safe-text text-white/45">Katselut</div><div className="mt-1 text-xl">{totalWatches}</div></div>
      </div>

      {topPost && (
        <div className="mt-5 rounded-[26px] border border-yellow-300/25 bg-yellow-300/10 p-4">
...
export default function FeedPageClean() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("media-feed-clean-v2-insanity-lite")
  ...
  return (
    <div className="h-[100dvh] overflow-hidden bg-[#050816] text-white">
      ...
      {event && (
        <div className="xp-pop fixed left-1/2 top-24 z-[90] w-[calc(100%-32px)] max-w-sm rounded-[26px] border border-cyan-300/30 bg-black/78 px-5 py-4 text-center text-sm font-black text-white shadow-2xl shadow-cyan-400/20 backdrop-blur-xl">
          {event.text}
        </div>
      )}
      ...
      <main id="feed-scroll-root" ref={scrollerRef} className="h-[100dvh] snap-y snap-mandatory overflow-y-auto scroll-smooth px-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <section data-feed-card className="snap-card mx-auto flex min-h-[100dvh] max-w-md items-start pb-[164px] pt-6">
          <FeedTopCard watchPot={watchPot} activePlayers={activePlayers} postCount={posts.length} totalWatches={totalWatches} topPost={topPost} />
        </section>

        {loading && (
          <section data-feed-card className="snap-card mx-auto flex min-h-[100dvh] max-w-md items-center pb-[164px]">
            <div className="w-full rounded-[34px] border border-cyan-200/15 bg-white/[0.075] p-8 text-center font-black shadow-2xl backdrop-blur-2xl">Feed latautuu...</div>
          </section>
        )}

        {!loading && posts.length === 0 && (
          <section data-feed-card className="snap-card mx-auto flex min-h-[100dvh] max-w-md items-center pb-[164px]">
            <div className="w-full rounded-[34px] border border-white/10 bg-white/[0.075] p-8 text-center shadow-2xl backdrop-blur-2xl">
        ...
        {!loading && posts.map((post, index) => (
          <section key={post.id} data-feed-card className="snap-card mx-auto flex min-h-[100dvh] max-w-md items-start pb-[164px] pt-6">
...
function PostCard({ post, index, voted, onVote, onShare, onImageStart, onImageWatch, onVideoTime, active }) {
  ...
  return (
    <article onDoubleClick={() => !voted && onVote(post)} className={`w-full overflow-hidden rounded-[36px] border bg-white/[0.075] p-[2px] shadow-2xl backdrop-blur-2xl transition-all duration-300 ${active ? "scale-100 opacity-100" : "scale-[0.975] opacity-90"} ${index === 0 ? "border-yellow-300/45 shadow-yellow-300/10" : "border-cyan-200/15 shadow-cyan-400/10"}`}>
      <div className="rounded-[34px] bg-[#08111f]/92 p-5">
        <div className="flex items-start justify-between gap-3">
          <div className={`grid h-16 w-16 shrink-0 place-items-center rounded-3xl text-xl font-black ${index === 0 ? "bg-yellow-300 text-black shadow-xl shadow-yellow-300/30" : "bg-cyan-500 text-white shadow-xl shadow-cyan-500/25"}`}>{rankBadge(index)}</div>
          <div className="min-w-0 flex-1 text-right">
            <p className="safe-text text-xs font-black uppercase text-cyan-100">{mediaUrl ? mediaType === "video" ? "🎥 Video" : "🖼️ Kuva" : "💬 Teksti"}</p>
            <p className="mt-1 safe-text text-xs font-black text-white/45">Score {Math.round(post.score || 0)} · Viral {viralMultiplier}x</p>
          </div>
        </div>

        {mediaUrl && (
          <div className="mt-5 overflow-hidden rounded-[28px] border border-white/10 bg-black/35 shadow-2xl shadow-black/30">
            {mediaType === "video" ? (
              <video src={mediaUrl} className="max-h-[42dvh] w-full object-cover" autoPlay muted loop playsInline controls={false} onPlay={() => onImageStart(post)} onTimeUpdate={(event) => onVideoTime(event, post)} />
            ) : (
              <img src={mediaUrl} alt="Post media" className="max-h-[42dvh] w-full object-cover" onLoad={() => { onImageStart(post); setTimeout(() => onImageWatch(post), 3000); }} onTouchStart={() => { onImageStart(post); setTimeout(() => onImageWatch(post), 3000); }} />
            )}
            <div className="border-t border-white/10 px-4 py-3 text-xs font-black text-cyan-100/80">👀 3s katselu → +XP · kasvattaa pottia</div>
          </div>
        )}
        ...
        {!mediaUrl && (
          <div className="mt-5 rounded-[26px] border border-cyan-200/15 bg-cyan-300/10 p-4 text-sm font-black text-cyan-50/90">
            🇫🇮 Tekstipostaus mukana kilpailussa. Double tap antaa äänen nopeasti.
          </div>
        )}

        <div className="mt-5 grid grid-cols-4 gap-2 text-center text-[10px] font-black">
          <div className="rounded-2xl bg-black/32 p-2.5"><div className="safe-text text-white/45">ÄÄNET</div><div className="mt-1 text-lg">{post.votes}</div></div>
          <div className="rounded-2xl bg-black/32 p-2.5"><div className="safe-text text-white/45">KATSELUT</div><div className="mt-1 text-lg text-pink-300">{post.watch_time_total || 0}</div></div>
          <div className="rounded-2xl bg-black/32 p-2.5"><div className="safe-text text-white/45">JAOT</div><div className="mt-1 text-lg text-cyan-200">{post.shares || 0}</div></div>
          <div className="rounded-2xl bg-black/32 p-2.5"><div className="safe-text text-white/45">SIJA</div><div className="mt-1 text-lg">#{index + 1}</div></div>
        </div>

        <div className="mt-4 grid grid-cols-2 gap-3">
          <button type="button" onClick={() => onVote(post)} disabled={voted} className="rounded-[22px] bg-gradient-to-r from-cyan-400 to-blue-600 px-4 py-3.5 text-sm font-black text-white shadow-2xl shadow-cyan-500/25 transition active:scale-[0.98] disabled:bg-none disabled:bg-white/10 disabled:text-white/40">{voted ? "Äänestetty" : "Tykkää +XP"}</button>
          <button type="button" onClick={() => onShare(post)} className="rounded-[22px] bg-gradient-to-r from-pink-500 to-fuchsia-600 px-4 py-3.5 text-sm font-black text-white shadow-2xl shadow-pink-500/25 transition active:scale-[0.98]">Jaa +Boost</button>
        </div>
