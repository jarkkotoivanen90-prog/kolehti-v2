...
export default function ProfilePageClean() {
  ...
  useEffect(() => {
    return onXPEvent((event) => {
      ...
      setXp((current) => Math.max(0, current + Number(event.amount || 0)));
    });
  }, []);
  ...
  async function load() {
    ...
    try {
      ...
      if (profileData?.xp !== undefined && profileData?.xp !== null) {
        setXp(Number(profileData.xp) || 0);
      }
    } catch {}
  }
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm font-black text-white/66">
                Level {level} · {progress}% seuraavaan
              </p>
            </div>

            <div className="grid h-20 w-20 shrink-0 place-items-center rounded-[28px] border border-cyan-100/10 bg-cyan-300/10 text-4xl shadow-[0_0_24px_rgba(21,131,255,.16)]">
              ⚡
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <div className="mt-4 space-y-3">
              <button
                ...
                className={`${innerPanel} w-full px-4 py-4 text-left font-black text-white active:scale-[0.98]`}
              >
                ⚙️ Sovelluksen asetukset
              </button>
        ...
        <section className={`${panel} mt-4`}>
          ...
          <div className="relative">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-2xl font-black text-glass">
                Omat postaukset
              </h2>

              <span className="rounded-full border border-cyan-100/10 bg-cyan-300/10 px-3 py-1 text-[10px] font-black uppercase text-cyan-100">
                {visiblePosts.length}
              </span>
            </div>
              ...
              {visiblePosts.length ? (
                visiblePosts.map((p) => (
                  <div key={p.id} className={`${innerPanel} p-4`}>
                    <p className="line-clamp-4 text-sm font-bold leading-relaxed text-white/82">
                      {p.content}
                    </p>
                  </div>
                ))
