...
import FeedTargetHint from "../../components/FeedTargetHint";
...
export default function FeedScreen() {
  ...
  useEffect(() => {
    const candidates = [activeIndex + 1, activeIndex - 1].filter(
      (index) => index >= 0 && index < posts.length
    );

    const fresh = candidates.filter(
      (index) => posts[index]?.id && !preloadedRef.current.has(posts[index].id)
    );
    ...
    cancelIdleTask(idleTaskRef.current);
    idleTaskRef.current = runWhenIdle(() =>
      fresh.forEach((index) => preloadPostMedia(posts[index]))
    );
  }, [activeIndex, posts]);

  async function load() {
    const startupFeed = await supabase.rpc("get_kolehti_startup_feed", {
      match_count: 80,
    });
    ...
    const aiFeed = await supabase.rpc("match_ai_feed", {
      match_count: 80,
    });
  ...
  function handleScroll(event) {
    ...
    scrollFrameRef.current = window.requestAnimationFrame(() => {
      ...
      const next = Math.max(
        0,
        Math.min(Math.round(scrollTop / height), posts.length - 1)
      );
      ...
      if (next !== lastIndexRef.current && previous) {
        ...
        if (!previous.bot) {
          supabase.rpc("record_ai_feed_signal", {
  ...
  async function likePost(post) {
    ...
    // XP + sound overlay, ei sotke feediä
    xpEvent("like", post.id, 2);
    ...
    supabase.rpc("record_kolehti_user_memory", {
      target_post_id: post.id,
      event_name: "like",
      signal_weight: 1,
    });
  ...
  function openMoneySheet(post) {
    haptic?.("tap");
    setSheet({ type: "money", post });
  }
  ...
  return (
    ...
    >
      ...
      <main
        ...
        className="h-full snap-y snap-mandatory overflow-y-auto overscroll-contain [scroll-behavior:auto] touch-pan-y [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      >
        ...
        {posts.map((post, index) => (
          <FeedCard
            ...
            onMoney={() => openMoneySheet(current)}
          />
        ))}
      </main>
      ...
      <AnimatePresence>
        ...
        {sheet?.type === "explain" && (
          <InfoSheet
            title="Miksi tämä näkyy?"
            label="AI"
            onClose={() => setSheet(null)}
          >
            <p className="text-sm font-bold leading-relaxed text-white/72">
              {sheet.post?.rank_reason ||
                "Tämä perustelu näkyy yhteisön signaalien, AI-arvion ja feedin vaihtelun perusteella."}
            </p>
        ...
        {sheet?.type === "money" && (
          <InfoSheet
            ...
            onClose={() => setSheet(null)}
          >
            <div className="rounded-3xl border border-yellow-200/18 bg-yellow-300/10 p-5 text-center text-4xl font-black text-yellow-50">
              {formatEuro(kolehti.dailyPot)}
            </div>
          </InfoSheet>
        )}
      </AnimatePresence>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="text-[10px] font-black uppercase tracking-[0.24em] text-cyan-100/58">
          {label}
        </div>
        ...
        <button
          type="button"
          onClick={onClose}
          className="mt-5 w-full rounded-2xl border border-white/10 bg-white/8 py-4 text-sm font-black uppercase tracking-[0.16em] text-white transition active:scale-[0.99]"
        >
          Jatka feediä
        </button>
      </motion.div>
    ...
    <section className="grid h-[100dvh] snap-start place-items-center bg-[#050816] px-6 text-center">
      <div className="rounded-[32px] border border-white/10 bg-white/6 p-8 backdrop-blur-xl">
        ...
        <p className="mt-2 text-sm font-bold text-white/55">
          Luo ensimmäinen perustelu ja käynnistä päivän potti.
        </p>
      </div>
    </section>
  );
...
function Toast({ message }) {
  return (
    <motion.div
      ...
      className="pointer-events-none fixed bottom-[calc(env(safe-area-inset-bottom)+100px)] left-1/2 z-[65] -translate-x-1/2 rounded-full border border-emerald-200/20 bg-emerald-400/16 px-4 py-2 text-xs font-black uppercase tracking-[0.16em] text-emerald-50 backdrop-blur-xl"
    >
      {message}
    </motion.div>
  );
