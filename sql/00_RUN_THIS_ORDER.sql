-- KOLEHTI V2 FINAL SQL RUN ORDER
-- Run these 4 steps in Supabase SQL Editor in this exact order.
-- This file is designed to avoid the errors you hit:
-- 1) missing referrals table
-- 2) missing optimizer_weights table
-- 3) ON CONFLICT key unique constraint error
...
create table if not exists public.group_members (id uuid primary key default gen_random_uuid());
create table if not exists public.posts (id uuid primary key default gen_random_uuid());
create table if not exists public.votes (id uuid primary key default gen_random_uuid());
create table if not exists public.notifications (id uuid primary key default gen_random_uuid());
create table if not exists public.retention_events (id uuid primary key default gen_random_uuid());
create table if not exists public.daily_winners (id uuid primary key default gen_random_uuid());
create table if not exists public.boost_events (id uuid primary key default gen_random_uuid());
create table if not exists public.user_events (id uuid primary key default gen_random_uuid());
create table if not exists public.referrals (id uuid primary key default gen_random_uuid());
create table if not exists public.growth_events (id uuid primary key default gen_random_uuid());
create table if not exists public.jackpot_rounds (id uuid primary key default gen_random_uuid());
create table if not exists public.reward_events (id uuid primary key default gen_random_uuid());
create table if not exists public.optimizer_runs (id uuid primary key default gen_random_uuid());
create table if not exists public.optimizer_weights (id uuid primary key default gen_random_uuid());
create table if not exists public.segment_stats (id uuid primary key default gen_random_uuid());
...
-- =====================================================
-- STEP 3: INDEXES, FUNCTIONS, TRIGGERS, RLS
-- =====================================================
...
create or replace function public.increment_user_xp(target_user_id uuid, xp_amount integer, reason text default 'action')
returns public.profiles as $$
declare
  updated_profile public.profiles;
begin
  if target_user_id is null or xp_amount is null then return null; end if;
  ...
  values (target_user_id, 'xp_gain', jsonb_build_object('amount', xp_amount, 'reason', reason, 'new_xp', updated_profile.xp, 'new_level', updated_profile.level));
...
begin
  ...
  values (new.id, split_part(new.email, '@', 1), coalesce(new.raw_user_meta_data->>'name', split_part(new.email, '@', 1)), 0, 1, public.make_referral_code(new.id))
  on conflict (id) do nothing;
  return new;
end;
$$ language plpgsql security definer set search_path = public;
...
-- =====================================================
-- STEP 4: SEED DATA WITHOUT ON CONFLICT(key)
-- =====================================================
...
select v.key, v.value, v.reason, '{}'::jsonb
from (
  values
    ('vote_weight', 1, 'Default vote signal weight'),
    ('invite_weight', 1, 'Default invite signal weight'),
    ('impression_weight', 1, 'Default impression signal weight'),
    ('notification_weight', 1, 'Default notification signal weight'),
    ('freshness_weight', 1, 'Default freshness signal weight'),
    ('quality_weight', 1, 'Default AI quality signal weight')
) as v(key, value, reason)
where not exists (
  select 1 from public.optimizer_weights ow where ow.key = v.key
);

insert into public.profiles (id, username, display_name, xp, level, referral_code)
select u.id, split_part(u.email, '@', 1), split_part(u.email, '@', 1), 0, 1, public.make_referral_code(u.id)
from auth.users u
where not exists (
...
-- DONE: after this, test XP by creating a post, voting, or claiming daily reward.
